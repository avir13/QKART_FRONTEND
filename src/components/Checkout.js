...
const OrderDetails = ({totalQty, totalCartValue, shippingCharges}) => {
  return (
    <Card sx={{ maxWidth: 345 }}>
      <CardContent>
        <Typography sx={{fontWeight: "bold"}} variant="h5" component="div">Order Details</Typography>
      </CardContent>
      <CardContent>
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      let res = await axios.post(`${config.endpoint}/user/addresses`, {"address": newAddress},{
        headers: {
          Authorization: `Bearer ${token}`
        }
      })
  ...
  const validateRequest = (items, addresses) => {
    if(parseInt(getTotalCartValue(items)) > parseInt(localStorage.getItem("balance"))) {
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        {
          variant: "error",
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      let balanceAfterCheckout = parseInt(localStorage.getItem("balance")) - parseInt(getTotalCartValue(items));
      localStorage.setItem("balance" , balanceAfterCheckout);
  ...
  let addressesToDisplay = (addresses.all.map((item) => (
    <Box
      key={item._id}
      onClick={(e) => {
        setAddresses({
    ...
    <>
      ...
      <Grid container>
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               ...
               { token ? (
                addresses.all.length && addressesToDisplay
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
         ...
         {items &&  <OrderDetails 
            totalQty={getTotalItems(items)} 
            totalCartValue={getTotalCartValue(items)}
            shippingCharges={0}
            />}
        </Grid>
      </Grid>
