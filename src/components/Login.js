...
const Login = () => {
  ...
  const login = async (formData) => {
    let userData = {
     ...
     try{
      setIsLoading(true);
      let data = await axios.post(`${config.endpoint}/auth/login`, userData);
      if (data.status === 201) {
  ...
  if (isLoading) {
    userLogin = <Box sx={{display: "flex", justifyContent: "center", alignItems: "center"}}><CircularProgress /></Box>;
  } else {
    userLogin = (
          <Button 
            className="button" 
            variant="contained"
            onClick={() => {
              let userInput = {"username": userName, "password": pwd};
